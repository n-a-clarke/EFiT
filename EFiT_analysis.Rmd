...
efit <- read_xlsx("EFiT_Main data set_28FEB2020.xlsx") %>% 
  filter(!partNum == 3, !partNum ==30, !partNum == 79) %>% 
  drop_na(CRIq)#remove participants no.3 failed screen, no.30 had to withdraw, no.79 may have unreliable results 
...
efit <- efit %>% 
  mutate(nls_correct_scaled = scale(nls_correct),
         ...
         matrixReason_scale = scale(matrixReason),
