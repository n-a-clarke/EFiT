...
nm_scm_paf <- lm(thi_total ~ nm_factor_score + PTA_Better + core_total, data = testing)

summary(nm_scm_paf)
```

```{r bayes scm nm factor regression}
#nm_thesis unscaled model
...
#single factor no roation
efit_paf <- fa(paf_efit_dat, nfactors = 1, fm = "pa", rotate = 'oblimin', covar = FALSE)
...
testing2 <- cbind(efit_factor_scores, efit_scm)

test_mod2 <- lm(THI ~ efit_factor_scores + hearThresh + glo_psych, data = testing2)
