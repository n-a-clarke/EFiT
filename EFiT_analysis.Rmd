...
mild_control_mild_modsev_cohensd_ci[1:9,2:4] <- mild_control_mild_modsev_cohensd_ci[1:9,2:4]*-1

mild_control_mild_modsev_cohensd_ci$group <- as.character(mild_control_mild_modsev_cohensd_ci$group)

mild_control_mild_modsev_cohensd_ci <- mild_control_mild_modsev_cohensd_ci %>% mutate(group = replace(group, group == "control vs tinnitus", "tinnitus vs control"))
...
ggplot(aes(x = fct_rev(task), y = cohens_d, colour = group)) +
  ...
  ylab("Standardised Mean Difference") +
  labs(colour = "Comparison Groups") +
  ggtitle("Control and Bothersome Tinnitus Group Task Performance Compared to Tinnitus Group")
