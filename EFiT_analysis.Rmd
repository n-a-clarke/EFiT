...
```{r prepare EFiT data}
efit_scm <- efit %>% 
  filter(THI_cat != "control")#remove control group
...
##effect of duration - tinnitus duration added into m
```{r prepare duration in each dataset and report removed missing}

```

```{r nm_data principe axis factor analysis}
...
nm_paf <- fa(pc_nm_dat, nfactors = 1, fm = "pa", rotate = 'oblimin', covar = FALSE)
...
nm_scm_fsr <- lm(thi_total ~ nm_factor_score + PTA_Better + core_total, data = testing)
...
```{r EFiT principle axis factor analysis}
#create dateset to reduce
...
#single factor no roation
efit_paf <- fa(paf_efit_dat, nfactors = 1, fm = "pa", rotate = 'varimax', covar = FALSE)
#stats
fa.stats(f = efit_paf)

rela::paf(paf_efit_dat)
#single factor no roation
efit_fa_ml <- fa(paf_efit_dat, nfactors = 1, fm = "ml", rotate = 'oblimin', covar = FALSE)

fa.stats(f = efit_fa_ml)
...
efit_scm_fsr_lm <- lm(THI ~ efit_factor_scores + hearThresh + glo_psych, data = testing2)
